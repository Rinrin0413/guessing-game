...
fn main() {
    println!("出力されゑ数價を的中(しなさい)"); // 数を当ててごらん
    println!("早ㄑ入力レて！"); // ほら、予想を入力してね

    // rand::thread_rng関数は gen_rangeメソッド(乱数生成器)を返す
    // gen_rangeメソッドは 第1引数-1 から 第2引数-1 までの乱数を生成する(OSからシード値を得て生成。)
    let secret_number = rand::thread_rng().gen_range(1, 101);

    println!("The secret number is: {}", secret_number); //臨時臨時臨時
    ...
    let guess: u32 = guess.trim().parse()
        // parseメソッドはエラりやすいので expectメソッドしてあげる
        .expect("\n贵樣！\n数價を入力(しなさい)");

    // この {} はプレースホルダーという
    println!("贵樣ゐ予想: {}", guess); // 次のように予想しました: {}
    ...
    match guess.cmp(&secret_number) {
        Ordering::Less => println!("矮小过ぎゑてあゑ"), //小さすぎ！
        Ordering::Greater => println!("过巨大だ！！"), //大きすぎ！
        Ordering::Equal => println!("贵樣ゐ胜利てず。\n賞金ゐ1000000000000元を贈呈ずゑ！！"), //やったね！
    }
}